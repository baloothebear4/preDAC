...
def ping(sock, message):
    ...
    # print( _print'received:\t"%s"' % received
...
def client(server, string):
    ...
    print( 'connecting to server "%s" on port: %s' % server_address)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['/?', '-help', '--help']:
        # if len(sys.argv) < 2:
        print(f"Usage: {os.path.basename(sys.argv[0])} [server IP] [message]")
        print("If server is not specified, {0} will open a port to act as the server".format(os.path.basename(sys.argv[0])))
        print("If message is not specified, a portion of 'Lorem Ipsum' is used")
        sys.exit(0)
